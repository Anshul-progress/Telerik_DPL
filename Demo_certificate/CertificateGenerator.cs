    ...
    public class CertificateGenerator
    {
        ...
        public async Task<string> GenerateCertificateAsync(CertificateViewModel model)
        {
            ...
            if (string.IsNullOrWhiteSpace(model.DocumentQr))
            {
                ...
                string qrFilePath = QrCodeService.GenerateQrFilePath("https://www.google.com", outDir, 800);

                byte[] qrImageBytes = File.ReadAllBytes(qrFilePath);
                string base64 = Convert.ToBase64String(qrImageBytes);
                model.DocumentQr = $"<img class='qr-code' alt='QR Code' src='data:image/png;base64,{base64}' style='width: 100px; height: 100px; border: 1px solid #cccccc; display: block;' />";
            ...
            try
            {
                ...
                // Search up the directory tree for the project root (where .csproj is)
